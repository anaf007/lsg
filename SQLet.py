...
class Connect:
    ...
    def __init__(self,user='root',password="cnpl3815241",database='lsg',port=3306):
        try:
            self._conn = MySQLdb.connect(host='192.168.2.102',user=user,passwd=password,db=database,port=3306,charset="utf8")
...
#         print sql
    ...
    def _sql_query(self,data,sql):
        ...
        for item in self._cursor:
            result.append(item)
        return result
#         column_names = self._cursor if data=='*' else tuple(data.split(','))
